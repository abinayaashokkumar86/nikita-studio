  ...
  return (
    <section id="masterclasses" className="py-24 px-6 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto max-w-5xl">
        <p className="text-teal uppercase tracking-[0.3em] text-sm font-medium text-center mb-3">{siteContent.masterclass.eyebrow}</p>
        <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-gold text-center mb-4">
          {siteContent.masterclass.title}
        </h2>
        <p className="text-cream/60 text-lg text-center max-w-2xl mx-auto mb-16">
          {siteContent.masterclass.description}
        </p>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {siteContent.masterclass.features.map((f, i) => {
        ...
        <div className="max-w-lg mx-auto bg-card border border-border rounded-2xl p-8 shadow-gold">
          <h3 className="text-foreground font-display text-2xl font-bold text-center mb-2">{siteContent.masterclass.notifyTitle}</h3>
          <p className="text-muted-foreground text-sm text-center mb-6">{siteContent.masterclass.notifyDescription}</p>
          {submitted ? (
            <p className="text-teal text-center font-medium">{siteContent.masterclass.successMessage}</p>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
              <input
                type="email"
                required
                placeholder={siteContent.masterclass.emailPlaceholder}
                value={email}
              ...
              <button type="submit" className="gradient-gold text-primary-foreground px-6 py-3 rounded-lg font-semibold text-sm uppercase tracking-wider hover:opacity-90 transition-opacity whitespace-nowrap">
                {siteContent.masterclass.submitLabel}
              </button>
            </form>
          )}
        </div>
